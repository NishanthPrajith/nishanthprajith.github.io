...
function getTargetRect(aspectRatio: number) {
  ...
  const maxHeight =
    viewportHeight * 0.85 - horizontalPadding * 2 - captionSpace;
...
export function GalleryPreview({
  ...
  const previewSrc = item.photo?.thumbnail ?? item.image;
  ...
  const upgradeToFullRes = useCallback(async () => {
    if (
      ...
      item.photo?.thumbnail === item.image ||
      !imgRef.current
    ) {
      return;
    }
  ...
  }, [item.image, item.photo?.thumbnail]);
  ...
  useEffect(() => {
    ...
    if (!item.photo?.thumbnail || item.photo?.thumbnail === item.image) {
      return;
    }
    ...
    >
      ...
      <div className="gallery-viewer-stage">
        <div ref={cardRef} className="gallery-viewer-card">
          ...
          <motion.div
            className="gallery-viewer-caption frosted-effect"
            initial={false}
            animate={
          ...
          >
            <p className="gallery-viewer-location">
              <i className="fa-solid fa-location-pin"></i>
              {item.photo?.location ?? item.group}
            </p>
            {item.title && (
              <p className="gallery-viewer-description">{item.title}</p>
            )}
          </motion.div>
        </div>
      </div>
